...
from sourcegraph_repository import (
    SourcegraphClient,
    SourcegraphError,
    build_searches,
    evidence_answer,
    symbols_from_locations,
)
...
def render_week4_submission(app_url, project_root, post_json):
    ...
    if "week4_repository_search" not in st.session_state:
        st.session_state.week4_repository_search = None
    ...
    with repository_tab:
        ...
        st.markdown("### Live Sourcegraph repository analysis")
        st.caption(
            "This search is separate from policy RAG. It queries the configured Sourcegraph repository directly "
            "and returns only live code-search evidence."
        )
        repository_question = st.text_input(
            "Repository question",
            placeholder="For example: Which function calls the retrieval service?",
            key="week4_repository_question",
        )
        analyze_repository = st.button(
            "Analyze Repository",
            type="primary",
            use_container_width=True,
            key="week4_analyze_repository",
        )
        if analyze_repository:
            try:
                client = SourcegraphClient()
                searches = build_searches(repository_question, client.config.repository)
                locations_by_search = []
                for search in searches:
                    locations_by_search.append({
                        **search,
                        "locations": client.search(search["query"]),
                    })
                all_locations = []
                seen_locations = set()
                for search in locations_by_search:
                    for location in search["locations"]:
                        location_key = (location.path, location.line, location.snippet)
                        if location_key not in seen_locations:
                            seen_locations.add(location_key)
                            all_locations.append(location)
                st.session_state.week4_repository_search = {
                    "question": repository_question,
                    "searches": locations_by_search,
                    "locations": all_locations,
                    "symbols": symbols_from_locations(all_locations),
                    "error": None,
                }
            except SourcegraphError as error:
                # Never substitute the static flow table or policy RAG for a
                # failed live request: that would look like a repository result.
                st.session_state.week4_repository_search = {
                    "question": repository_question,
                    "searches": [],
                    "locations": [],
        ...
        live_result = st.session_state.week4_repository_search
        if live_result:
            st.markdown("#### User question")
            st.write(live_result["question"])
            if live_result["error"]:
                st.error(live_result["error"])
            else:
                st.markdown("#### Sourcegraph search results")
                query_rows = [
                    {
                        "Search purpose": search["label"],
                        "Sourcegraph query": search["query"],
                        "Locations found": len(search["locations"]),
                    }
                    for search in live_result["searches"]
                ]
                st.markdown(_table_html(query_rows), unsafe_allow_html=True)
                locations = live_result["locations"]
                st.markdown("#### Relevant files, line ranges, and code references")
                if locations:
                    st.markdown(_table_html([
                        {
                            "File path": location.path,
                            "Line": location.line if location.line is not None else "Not provided",
                            "Reference": location.reference,
                        }
                        for location in locations
                    ]), unsafe_allow_html=True)
                    for location in locations:
                        with st.expander(f"{location.reference} — Sourcegraph code snippet"):
                            st.code(location.snippet or "Sourcegraph returned this location without a preview.", language="python")
                            if location.sourcegraph_url:
                                st.caption(f"Sourcegraph reference: {location.sourcegraph_url}")
                else:
                    st.info("Sourcegraph completed the search but returned no code locations.")
                st.markdown("#### Relevant functions and symbols")
                if live_result["symbols"]:
                    st.write(", ".join(f"`{symbol}`" for symbol in live_result["symbols"]))
                else:
                    st.info("No function or class name was visible in the returned Sourcegraph snippets.")
                st.markdown("#### Repository-level answer")
                st.write(evidence_answer(live_result["question"], locations, live_result["symbols"]))

        st.markdown("**Sourcegraph configuration:** set `SOURCEGRAPH_REPOSITORY` and `SOURCEGRAPH_ACCESS_TOKEN`; optionally set `SOURCEGRAPH_URL` or `SOURCEGRAPH_GRAPHQL_URL`. Credentials are never stored in the dashboard or code.")
